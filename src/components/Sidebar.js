...
export default function Sidebar() {
  ...
  return (
    <div className="w-full h-[56px] md:w-[60px] md:h-full bg-[#080b12] border-t md:border-t-0 md:border-r border-white/[0.06] flex flex-row md:flex-col justify-between items-center px-1.5 md:px-0 py-0 md:py-3 flex-shrink-0 z-30 select-none order-last md:order-first">
      
      <div className="flex flex-row md:flex-col items-center justify-around md:justify-start w-full md:w-auto flex-1 md:flex-none md:gap-0.5 h-full md:h-auto">
        ...
        <motion.div 
          ...
          whileTap={{ scale: 0.9 }}
          className="hidden md:flex w-9 h-9 rounded-lg items-center justify-center cursor-pointer mb-3 relative overflow-hidden"
        ...
        <div className="hidden md:block w-7 h-px bg-white/[0.04] mx-auto mb-1" />
        ...
        {navItems.map((item) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.85 }}
              className={`relative w-10 h-10 md:w-10 md:h-10 flex items-center justify-center rounded-lg cursor-pointer transition-all duration-150 ${
                isActive 
                  ? 'text-accent' 
            ...
            >
              ...
              {isActive && (
                <motion.div
                  layoutId="nav-bar"
                  className="hidden md:block absolute left-0 top-2 bottom-2 w-[2px] rounded-r-full bg-accent"
              ...
              <AnimatePresence>
                {hovered === item.id && (
                  <motion.span
                    ...
                    exit={{ opacity: 0, x: -6 }}
                    className="hidden md:block absolute left-[52px] bg-[#111827] border border-white/[0.08] text-[11px] text-white/80 px-2.5 py-1 rounded-md shadow-xl z-[60] font-medium"
                  >
                    {item.label}
                  </motion.span>
                )}
        ...
        <div className="hidden md:block w-7 h-px bg-white/[0.04] mx-auto my-1" />
        ...
        >
          ...
          <AnimatePresence>
            {hovered === 'term' && (
              <motion.span initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -6 }} className="hidden md:block absolute left-[52px] bg-[#111827] border border-white/[0.08] text-[11px] text-white/80 px-2.5 py-1 rounded-md shadow-xl z-[60] font-medium">Console</motion.span>
            )}
          </AnimatePresence>
        </motion.button>
      ...
      {/* Bottom */}
      <div className="flex flex-row md:flex-col items-center md:gap-1.5 gap-1.5 px-1 md:px-0">
        {currentSong && (
          <div className="hidden md:flex items-center justify-center w-10 h-5">
        ...
        {user ? (
          <>
            <motion.button onClick={() => handleNav('/auth')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.9 }} className="relative w-9 h-9 md:w-10 md:h-10 flex items-center justify-center cursor-pointer">
            ...
            <motion.button onClick={logout} whileTap={{ scale: 0.85 }} className="hidden md:flex w-10 h-10 items-center justify-center text-[#4a5568] hover:text-red-400/80 rounded-lg cursor-pointer transition-colors">
              <LogOut size={15} />
            </motion.button>
          </>
        ) : (
