...
const SectionHeader = ({ title, path }) => {
  ...
  if (isGlass) {
    cleanTitle = title
      .replace(/_/g, ' ')
      .replace('Trending Modules', 'Trending Songs')
      .replace('Fresh Compiles', 'New Releases')
      .replace('Curated Packages', 'Curated Playlists');
  }

  return (
    <div className="flex items-center justify-between mb-6 border-b border-border-color/30 pb-3">
      <div className="flex items-center gap-3">
        <div className={`w-1 h-6 bg-accent ${isGlass ? 'rounded-full' : 'shadow-[0_0_10px_var(--accent)]'}`} />
        <div>
          <h3 className="text-xs md:text-sm font-black uppercase tracking-[0.2em] text-text-primary/90">{cleanTitle}</h3>
          {!isGlass && <p className="text-[8px] text-text-secondary/35 font-mono mt-0.5">LOCATION: {path}</p>}
        </div>
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-12 select-none animate-pulse">
        <div className="w-full h-64 md:h-80 rounded-3xl bg-bg-secondary/40 border border-border-color/30 flex items-end p-8">
          <div className="flex flex-col gap-3 max-w-md w-full">
            <div className="w-24 h-3 bg-text-secondary/15 rounded" />
            <div className="w-3/4 h-8 bg-text-secondary/15 rounded" />
            <div className="w-1/2 h-4 bg-text-secondary/15 rounded" />
          </div>
        </div>
        <div>
          <div className="w-48 h-6 bg-bg-secondary/40 rounded-md mb-6" />
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {[1, 2, 3, 4, 5, 6].map(i => (
              <div key={i} className="flex flex-col gap-3">
                <div className="w-full aspect-square bg-bg-secondary/40 border border-border-color/30 rounded-2xl" />
                <div className="w-3/4 h-3 bg-text-secondary/15 rounded" />
                <div className="w-1/2 h-2.5 bg-text-secondary/15 rounded" />
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="flex flex-col gap-16 select-none pb-20">
      ...
      <section className="relative h-64 md:h-80 rounded-3xl overflow-hidden glass-panel flex items-end p-8 group">
         ...
         <div className="absolute top-0 right-0 p-8 z-0 opacity-10 group-hover:opacity-20 transition-opacity">
            {isGlass ? (
              <Disc size={200} className="text-accent animate-spin-slow" />
            ) : (
         ...
         <div className="relative z-20 max-w-2xl">
            <div className="flex items-center gap-2 text-accent text-[10px] font-bold tracking-widest uppercase mb-4">
               ...
               {isGlass ? 'Featured Spotlight' : 'Kernel_Priority_Stream'}
            </div>
            <h1 className="text-3xl md:text-5xl font-black text-text-primary leading-tight mb-4 tracking-tighter">
              {isGlass ? 'UNLEASH THE ' : 'UNLEASH THE '}<span className="text-accent">{isGlass ? 'MUSIC.' : 'WAVEFORMS.'}</span>
            </h1>
            <p className="text-sm text-text-secondary max-w-md font-semibold leading-relaxed font-sans">
              {isGlass 
                ? 'High-fidelity audio stream. Experience ultimate clarity with our premium web player.'
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {data.albums.map((album) => (
              <div 
                ...
                className="group relative flex flex-col p-3 rounded-2xl border bg-bg-secondary/40 border-border-color/30 hover:border-accent/40 hover:bg-bg-secondary/70 transition-all duration-500 cursor-pointer"
              >
                ...
                <div className="flex items-center gap-1.5 mt-0.5 text-text-secondary/70">
                  <Disc size={10} />
